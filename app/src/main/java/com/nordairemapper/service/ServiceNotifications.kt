...
object ServiceNotifications {
    ...
    fun notifyOpenAfterBoot(context: Context) {
        ...
        val notification = Notification.Builder(context, CHANNEL_ID)
            .setSmallIcon(R.drawable.ic_launcher_foreground)
            .setContentTitle("Open Keyforge once to restore Plus Key detection")
            .setContentText("After a reboot, Android waits for one foreground launch. Tap — detection recovers automatically.")
            .setContentIntent(intent)
            .setAutoCancel(true)
            .setOngoing(false)
            .build()
