...
| `service/DebugTrace.kt` (new) | file-based diagnostic traces (auto_pause.log) | 10 |
| `LogcatWatcherService.kt` / `ReadLogsGrantViaWirelessAdb.kt` / `ShizukuGrant*.kt` / `PairingSession.kt` / `PairingReplyReceiver.kt` / `ActionFeedbackOverlayService.kt` / `BootReceiver.kt` / `AccessibilityAutoResumeService.kt` / `PlusKeyAccessibilityService.kt` / `LearningMode.kt` | audit #2 fixes | 09, 10 |
| `EnableDetectionViewModel/Screen`, `DeveloperScreen`, `KeyLearning*`, `Home*`, `Remap*`, `AppPickerSheet`, `Exclusions*`, `OverlaySettings*`, `Backup*`, `NordNavHost`, `OnboardingScreen`, `PressType` | presentation audit #2 fixes | 08–10 |
